...
!
PROGRAM MAIN
  USE DG_LIN_CONVEC_MODULE, ONLY: DG_LIN_CONVEC
  USE GTINPT_MODULE, ONLY: GTINPT
...
!
  ...
  INTEGER NGEOM, NELEM, NPOIN, NBOUN, MXSID
  INTEGER ILINE, NTIME, VNPNT, FORCEOUT
...
!
    CALL GTINPT(NDIMN=NDIMN, NNODE=NNODE, NPOIN=NPOIN, &
                NELEM=NELEM, NBOUN=NBOUN, INTMA=INTMA, &
                COORD=COORD, BSIDO=BSIDO, IELSI=IELSI, &
                NBNOI=NBNOI, MPI_RANK_V=MPI_RANK_V)
...
!
 ...
 &          'NOTE: YOU HAVE CHOSEN TO USE CONSISTENT MASS MATRICES'
      ...
      WRITE (*, *)
      CLOSE (15)
    ENDIF  ! IF(MPI_RANK.EQ.0)THEN !
  ENDIF  !  IF(MPI_RANK_P.EQ.0)THEN !

!
! *** CALL DG_LIN_CONVEC FOR THE DISTRIBUTION FUNCTION CONVECTION PROCESS
! *** (CALLED BY ALL PROCESSORS RANK 0 -> MPI_SIZE-1)
  CALL DG_LIN_CONVEC(NDIMN=NDIMN, NNODE=NNODE, NPOIN=NPOIN, &
                     NELEM=NELEM, NBOUN=NBOUN, INTMA=INTMA, &
                     COORD=COORD, NTIME=NTIME, CSAFM=CSAFM, &
                     IMMAT=IMMAT, NBNOI=NBNOI, BSIDO=BSIDO, &
                     NBNOR=NBNOR, NGEOM=NGEOM, RORDER=RORDER, &
                     TORDER=TORDER, VNPNT=VNPNT, SUMWEIGHT=SUMWEIGHT, &
                     VCORD=VCORD, rv=rv, ALPHA=ALPHA, &
                     CINF=CINF, MXSID=MXSID, IVD=IVD, &
                     FORCEOUT=FORCEOUT, d=d, R=R, &
...
!
  IF (MPI_RANK .EQ. 0) THEN
    PRINT *, &
      'THANK YOU FOR USING BENS BOLTZMANN SOLVER'
