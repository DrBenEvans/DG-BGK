       SUBROUTINE FLUCON2(NELEM,VNPNT,DISNF_PP,NBNOI,NBOUN_PP,& 
     &                  BSIDO_PP,NBNOR,RSIDO_PP,VCORD,rv,& 
     &                 ETA,NSIDE,ISIDE_PP,RORDER,TORDER,SUMWEIGHT,R,&
...
!
      ...
      include 'mpif.h' 
! 
      INTEGER NELEM,VNPNT,NBNOI,NBOUN_PP,IP1,IP2 
      INTEGER NBNOR,IB,IV,IS,IV1,IV2,IETA,IZETA 
      INTEGER NSIDE,RORDER,TORDER,IV3,IV4
      INTEGER BSIDO_PP(NBNOI,NBOUN_PP),IEL,ISIDE_PP(8,NSIDE) 
...
! 
          DO IS=1,NSIDE !dkljsdfaadfda
            ...
            ELSEIF((IP2.EQ.IP1T).AND.(IP1.EQ.IP2T))THEN 
              ...
              EXIT
            ENDIF 
          ENDDO ! DO IS=1,NSIDE !dkljsdfaadfda
! 
...
!
      ...
      MPI_ALLREDUCE(SUMTOP_ARR,SUMTOPG_ARR,NBOUN_PP,MPI_REAL,MPI_SUM,&
     &      MPI_COMM_V,MPI_IERR)
