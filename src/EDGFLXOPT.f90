           SUBROUTINE EDGFLXOPT(NELEM,NSIDE,ISIDE,RHS,& 
...
! 
      ...
      INTEGER ISIDE(8,NSIDE),VNPNT,IV,OPPSLAVERANK,RORDER,TORDER 
...
!     COMMUNICATION STUFF (OPTIMIZATION)
      INTEGER :: SENDRECV_TOT_LENGTH_MAX, IDX 
      ...
      INTEGER, ALLOCATABLE :: SENDRECV_START_OFFSETS(:) ! IDX OF FIRST ELEMENT TO SEND TO ANY GIVEN RANK 
...
!     SETUP COMMUNICATION STUFF (OPTIMIZATION)
      ...
      ALLOCATE(SENDRECV_MAX_LENGTHS(NGRPS))
      ALLOCATE(SENDRECV_START_OFFSETS(NGRPS))
      ALLOCATE(SEND_LENGTHS(NGRPS))
      ALLOCATE(RECV_LENGTHS(NGRPS))
      
      DO IG=1,NGRPS
          SENDRECV_MAX_LENGTHS(IG) = GCOMM(RANK-1,IG)
      ENDDO
...
! *** NOW DEAL WITH PROCESSOR BOUNDARY EDGES HERE!!!!! 
               IS_PP = SDCOM_PP(1,IS)
               OPPSLAVERANK = SDCOM_PP(3,IS) - 1
               IS_OTHER = SDCOM_PP(2,IS)
               
               SEND_LENGTHS(OPPSLAVERANK)= &
     &                     SEND_LENGTHS(OPPSLAVERANK)+2
           ...
           WRITE(*,*) "DEBUG, edge not communicated"
           ENDIF
...
!
               ...
               IDX=SENDRECV_START_OFFSETS(OPPSLAVERANK)+&
     &                                SEND_LENGTHS(OPPSLAVERANK)-1
               SEND_EDGE_DATA(2*IDX+1)=RHSI(1)
               SEND_EDGE_DATA(2*IDX+2)=RHSI(2)
               SEND_EDGE_DATA_IDX(IDX)=FLAG*IS_OTHER ! STORING FLAG 
 ...
 1000 CONTINUE ! END OF CYCLE OVER SIDES
     ...
     &      MPI_COMM_SLAVES,MPI_IERR)
      ...
      DO IG=1,NGRPS
         DO IDX=SENDRECV_START_OFFSETS(IG)+1,&
     &              SENDRECV_START_OFFSETS(IG)+RECV_LENGTHS(IG) 
            IS = ABS(RECV_EDGE_DATA_IDX(IDX))
            IF(RECV_EDGE_DATA_IDX(IDX).LT.0) THEN 
