        SUBROUTINE EDGCOM(NGRPS,MPI_RANK_P,NCOMM_PP,NSIDE_PP,SDCOM_PP,ISIDE_PP,&
     &               LCOMM_PP,ISCOM_PP )
!
...
! 
       ...
       INCLUDE 'mpif.h'
!
       INTEGER IG,IS,IST,NGRPS,MPI_RANK_P,NCOMM_PP,I,MPI_IERR
...
! *** LOOP OVER EACH SIDE
!
        IF(MPI_RANK_P.EQ.IG)TMPNSIDE_PP=NSIDE_PP
        CALL MPI_BCAST(TMPNSIDE_PP,1,MPI_INTEGER,IG,MPI_COMM_P,MPI_IERR)
        IF(MPI_RANK_P.EQ.IG)FLAG=0
!
        DO 1100 I=1,TMPNSIDE_PP
         ...
         IF((IEL.EQ.-1).OR.(IER.EQ.-1))THEN
           CALL MPI_BCAST(OPPRANK,1,MPI_INTEGER,IG,MPI_COMM_P,MPI_IERR)
           CALL MPI_BCAST(GLOBIS,1,MPI_INTEGER,IG,MPI_COMM_P,MPI_IERR)
           IF(MPI_RANK_P.EQ.OPPRANK)THEN
