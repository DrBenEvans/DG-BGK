...
CONTAINS
  ...
  SUBROUTINE OUTPUT(NNODE, NPOIN, NELEM, &
                    maxNELEM_PP, COORD, INTMA, &
                    DISNF_PP, VNPNT, ELGRP, &
                    NEGRP, IVD, MPI_RANK_P, &
                    MPI_SIZE_P, MPI_COMM_P, MPI_RANK_V, &
                    MPI_SIZE_V, MPI_COMM_V, VSPACE_FIRST, &
                    VSPACE_LAST, VCORD, rv)
!
...
!
    INTEGER VNPNT, NPOIN, NELEM, NNODE, IP, IN, NPART
    INTEGER maxNELEM_PP
    INTEGER J, IE, MPI_IERR, MPI_STATUS(MPI_STATUS_SIZE)
    INTEGER IE_PP
    INTEGER IV, IVTRUE, ELGRP(NELEM, 2)
    REAL COORD(2, NPOIN)
...
!
    ...
    INTEGER MPI_RANK_V, MPI_SIZE_V, MPI_COMM_V, IVP
   ...
   &             MPI_COMM_V, MPI_STATUS, MPI_IERR)
          ...
          DO IV = VSPACE_FIRST, VSPACE_LAST ! 1 to VNPNT_PART for rank 0
            ...
            IF ((MOD(IV, PRINT_EVERY)) .EQ. 0) THEN
              WRITE (*, "(A23,I5)") "V-space iteration no.", IV
            ENDIF
