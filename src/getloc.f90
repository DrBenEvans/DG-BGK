MODULE GETLOC_MODULE
  IMPLICIT NONE
CONTAINS
  SUBROUTINE GETLOC(NSIDE, NGEOM, NELEM, &
                    maxNELEM_PP, NPOIN, NBNOI, &
                    NBOUN, NBNOR, NNODE, &
                    NGRPS, ELGRP, NEGRP, &
                    NX, NY, EL, &
                    GEOME, MMAT, CMMAT, &
                    INTMA, COORD, BSIDO, &
                    RSIDO, ISIDE, NELEM_PP, &
                    maxNBOUN_PP, NBOUN_PP, maxNPOIN_PP, &
                    NPOIN_PP, maxNSIDE_PP, NSIDE_PP, &
                    ISIDE_PP, NX_PP, NY_PP, &
                    EL_PP, GEOME_PP, MMAT_PP, &
                    CMMAT_PP, INTMA_PP, COORD_PP, &
                    BSIDO_PP, RSIDO_PP, IMMAT, &
                    MPI_RANK_P, LCOMM_PP, IPCOM_PP, &
                    IBCOM_PP, ISCOM_PP, NPGRP, &
    ...
    INTEGER TAG4, TAG5, SIZE4, TAG6, SIZE6, TAG7, SIZE7, TAG8
    ...
    INTEGER IS, IE, NELEM_G, CO, IEG, IG, IGT, I
    ...
    IF (MPI_RANK_P .EQ. 0) THEN ! vnsdsjkdkja
      ...
      DO IG = 1, NGRPS
        ...
        I = 0
        !
        DO E = 1, NELEM_G    !LOOP OVER THE PROCESSOR'S ELEMENTS
1002      CONTINUE
          I = I + 1
          IGT = ELGRP(I, 1)
          IF (IG .EQ. IGT) THEN
