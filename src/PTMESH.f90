        SUBROUTINE PTMESH(NELEM,INTMA,NPOIN,NPROC,NSIDE,ISIDE,& 
...
!
      ...
      INTEGER NELEM,NPOIN,NGRPS,NELEM_PP,GRPT,FLAG 
      INTEGER NPOIN_PP,NSIDE_PP,NGEOM,NBNOI,NBOUN,NBNOR 
      INTEGER NPROC,TAG,SIZE1,TAG1,TAG2,RANK 
      ...
      INTEGER NEGRP(NPROC-1),ISIDE(8,NSIDE),BSIDO(NBNOI,NBOUN) 
      INTEGER GCOMM(NPROC-1,NPROC-1) 
      ...
      INTEGER ISCOM_PP(maxNSIDE_PP),NPGRP(NPROC-1) 
...
!
      ALLOCATE(VFLAG(NPROC-1))
! 
      NGRPS=NPROC-1 
      IF(RANK.EQ.0)THEN  ! fkjfhkafda 
! 
...
! 
      CLOSE(12) 
! 
! *** INCREASE THE RANK BY 1 FOR EACH NODE SO THAT NO NODES/ELEMENT ARE 
! *** ALLOCATED TO PROCESSOR RANK 0 
! 
      DO 120 IP=1,NPOIN 
      NODPT(IP)=NODPT(IP)+1 
 120  CONTINUE 
...
! 
      ...
      NEGRP(GE)=NEGRP(GE)+1 
 103  CONTINUE 
      ENDIF     ! END IF RANK.EQ.0  fkjfhkafda
! 
...
! 
      ...
      ENDIF  
! 
! *** SEND NELEM_PP TO PROC RANK IG 
! 
     CALL MPI_SEND(NELEM_PP,1,MPI_INTEGER,IG,100,& 
     &     MPI_COMM_WORLD,MPI_IERR) 
! 
      ENDIF   !LINKS WITH IF STATEMENT ON LINE 94 
! 
! *** RECEIVE NELEM_PP ON PROC RANK IG 
! 
     IF(RANK.EQ.IG)THEN 
...
! 
      SIZE1=2*NELEM 
      CALL MPI_BCAST(ELGRP,SIZE1,MPI_INTEGER,0,MPI_COMM_WORLD& 
     &     ,MPI_IERR) 
! 
! *** INITIALISE GCOMM 
! 
...
! 
 ...
 107  CONTINUE 
      ...
      CALL MPI_BCAST(GCOMM,NGRPS*NGRPS,MPI_INTEGER,0,MPI_COMM_WORLD,&
     &        MPI_IERR)
!
     ...
     &      RANK,LCOMM_PP,IPCOM_PP,IBCOM_PP,ISCOM_PP,NPGRP,NCOMM_PP,&
     &          SDCOM_PP)
      CALL MPI_BARRIER(MPI_COMM_WORLD,MPI_IERR)
! 
! *** FORMAT STATEMENTS 
