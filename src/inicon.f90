MODULE INICON_MODULE
CONTAINS
  SUBROUTINE INICON(NPOIN, NNODE, NELEM, NELEM_PP, INTMA_PP, ELGRP,&
       &     NPOIN_PP, COORD_PP, VNPNT, VCORD, DISNF_PP, rv, IVD,&
       &                 MPI_RANK_P, MPI_COMM_P,&
...
!
    ...
    INTEGER IN, INV, RS, IV, NELEM, NPOIN, NNODE
    INTEGER IE, IP_PP, I, IE_PP, NELEM_PP
    INTEGER VNPNT, NPOIN_pp, INTMA_PP(NNODE, NELEM_PP)
    INTEGER TEST1, TEST2, NPART
    INTEGER ELGRP(NELEM, 2), MPI_IERR
!
    REAL VCORD(3, VNPNT), COORD_PP(2, NPOIN_PP)
    REAL T1, P1, GC, RHO1, n1, NA, ETA, ZETA, R, THETA
    REAL M, BETA1, TMP1, MAGX, rv, C
    REAL UX, UY, TMP, SPEED, PI, F0, XPOS
    REAL W, L
...
! ***    END LOOP OVER VELOCITY SPACE NODES
            ENDDO !DO INV = VSPACE_FIRST, VSPACE_LAST
